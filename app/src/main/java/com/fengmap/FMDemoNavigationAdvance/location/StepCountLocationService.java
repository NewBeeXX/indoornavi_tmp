...
//                oldStep=step;
//                //根据差计算步长，degreeSub调好了，degreeSub就是从正北为0，逆时针数起的角度。
//                Log.i("pig", "隔2s的步数差：" + stepSubb);
                //TODO 下面的30.0表示：在地图上 从入口进去的时候指南针的角度 30为预设
                Log.v("方向","****:"+lastDegree);
                double degreeSub=(/*30+90*/360.0+lastDegree)%360; //相对地球竖直向上↑的角度差 角度从逆时针算
                Log.v("方向","之后的degree"+degreeSub);
//                Log.i("zjx","degreeSub:"+degreeSub);
                int stepSub=0;
                if((stepSub=(step-stepLocation.lastAMapPoint.getStep()))!=0){
                    double xMap,yMap;
                    Intent intent = new Intent();
                    if(stepSub<0){  //貌似不会执行吧？stepSub单增
                        xMap=stepLocation.lastAMapPoint.getX();
                        yMap=stepLocation.lastAMapPoint.getY();
                    }
                    else {
                        //步长在这，stepsub是个dt，degreesub是角度数差,改了一下步长
                        double lenStep = stepSub * 0.4;//走的距离 需要转换
