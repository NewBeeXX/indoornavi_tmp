...
public class FMNavigationApplication extends BaseActivity implements
    ...
    private FMGeoCoord c = null;

    private FMGeoCoord lastPosition = null;


    boolean isLocating = false;


    public void locate(final FMGeoCoord position, final float angle) {
        updateHandledMarker(position, angle);
        updateNavigationOption();
        if (lastPosition != null && lastPosition.getGroupId() != position.getGroupId()) {
            onCrossGroupId(this.c.getGroupId(), position.getGroupId());


            //FMNavigationInfo var3 = this.createNavigationInfo(this.c, position, angle);
            if (false/*这里判断一下当前点到终点的距离*/) {
                mActualNavigation.stop();
                onComplete();
            }


            //抽出onwalking里的代码,不知道为什么跑在uithread会不起作用，暂时在同一进程跑
            ...
            // mActualNavigation.controlMapView(position, -angle);
            ...
            public void handleMessage(Message msg) {
                switch (msg.what) {
                    case 10086:
                        ...
                        Log.i("方向", "调locate时的ax:" + ax + "ay:" + ay + "degree:" + (degree + 360) % 360);
//                        FMNavigationApplication.mActualNavigation.locate(new FMGeoCoord(1,
//                                new FMMapCoord(ax, ay)),(degree+360)%360);

//                        FMGeoCoord position=new FMGeoCoord(1, new FMMapCoord(ax, ay)) ;
//                        float angle=(degree+360)%360;
//                        updateHandledMarker(position,angle);

                        locate(new FMGeoCoord(1, new FMMapCoord(ax, ay)), (degree + 360) % 360);
...
//            public void run() {
//                FMNavigationApplication.mActualNavigation.locate(new FMGeoCoord(1,
//                        new FMMapCoord(12961650.576796599, 4861814.63807118)),0);
//            }
//        },2000);
//        new Handler().postDelayed(new Runnable() {
//            @Override
//            public void run() {
//                FMNavigationApplication.mActualNavigation.locate(new FMGeoCoord(1,
//                        new FMMapCoord(12961650.576796599, 4861814.63807118)),90);
//            }
//        },4000);
    }
        ...
        public void onReceive(Context context, Intent intent) {
            ...
            switch (astate) {
                case 10086:
                    ...
                    if (isfirstLocation)
                        Toast.makeText(FMNavigationApplication.this, "定位成功了", Toast.LENGTH_SHORT).show();
                    break;
